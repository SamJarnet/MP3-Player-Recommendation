...
class SearchWindow:
    ...
    def openNewWindow(self):
        ...
        self.load_box = Listbox(master, selectmode="SINGLE", bg="darkgrey",
                                selectbackground="darkblue", width=30, height=28)
        self.load_box.place(x=10, y=10)

        self.scrollbar = Scrollbar(master, width=30)
        self.scrollbar.place(x=200, y=100)
        ...
        self.search_button = Button(
            master, text="Search", command=self.search_music)
        self.search_button.place(x=300, y=20)

        self.add_song_button = Button(
            master, text="Add song", command=self.add_song)
        self.add_song_button.place(x=300, y=60)

        self.show_info_button = Button(
            master, text="Show information", command=self.show_info)
        ...
        for i in range(0, 100):
            ...
            self.mergeSort(self.searched_songs)
    ...
    def add_song(self):
        self.mp3_player.playlist.insert(
            0, self.load_box.get(self.load_box.curselection()[0]))
        self.mp3_player.stored_songs[self.selected_playlist].insert(
            0, self.load_box.get(self.load_box.curselection()[0]))
        with open("playlists\\" + self.mp3_player.stored_playlists[self.selected_playlist] + self.mp3_player.login_manager.user + ".txt", "w") as file:
            for line in self.mp3_player.stored_songs[self.selected_playlist]:
                file.write("".join(line) + "\n")
        self.add_bias_predictions()
        self.mp3_player.remove_empty_lines(
            "playlists\\" + self.mp3_player.stored_playlists[self.selected_playlist] + self.mp3_player.login_manager.user + ".txt")

    # shows the artist and year of release of the selected song
    def show_info(self):
        print(self.searched_songs)
        self.info_box.delete(0, "end")
        artists = self.data[16][self.searched_songs[self.load_box.curselection()[
            0]]]
        artists = artists.replace("[", "")
        artists = artists.replace("]", "")
        artists = artists.split(",")
        year = str(
            int(self.data[1][self.searched_songs[self.load_box.curselection()[0]]]*2020))
    ...
    def search_music(self):
        ...
        for i in range(0, 170653):
            if self.data[15][i][0:len(text)].lower() == text.lower() or (self.data[15][i].lower().__contains__(text.lower())
                                                                         and (len(text)-(self.data[15][i].lower().find(text.lower())))**2 < 25 and len(text) > 2):
                self.searched_songs.insert(0, i)
                self.mergeSort(self.searched_songs)
                self.load_box.insert(0, self.data[15][i])
    ...
    def add_bias_predictions(self):
        for i in range(0, 14):
            ...
            self.mp3_player.song_score[self.mp3_player.selected_playlist] += self.data[i][self.searched_songs[self.load_box.curselection()[
                0]]]
    ...
    # uses the merge sort algorithm to sort by alphabetical
    ...
    def mergeSort(self, lst):
        if len(lst) > 1:
            ...
            while i < len(left) and j < len(right):  # traverse the two halves
                if left[i] <= right[j]:
                    lst[k] = left[i]
                    i += 1
                else:
            ...
            while i < len(left):  # traverse remaining halves
                lst[k] = left[i]
                i += 1
                k += 1
